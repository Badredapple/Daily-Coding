...
#define DEBUG_LISTS_LENGTHS_ONLY 0

using namespace std;

const int MAX_CITIES = 20;

enum ENUM_CITIES{Arad=0, Busharest, Craiova,Eforie, FBucharest, Craiova, Drobeta, Eforie, Fagaras, Giurgiu, Hirsova, Iasi, Lugoj, Mehadia, Neamt, Oradea, Pitesti, RimnicuVilcea, Sibiu, Timisoara, Urziceni, Vaslui, Zerind};
...
// Euclidean distance between "this" node city and Buchare
...
//generate the successor node of "this" node
...
bool PathSearchNode::GetSuccessors( AStarSearch<PathSearchNode> *astarsearch, PathSearchNode, *parent_node )
{
	...
	{
		if(RomaniaMap[city][c] < 0) contiune;
		NewNode = PathSearchNode((ENUM_CITIES)c);
		astarsearch - > AddSucessor( NewNode );
	}
	return true;
}
...
int main(int args, char *rgv[])
{
	//creating the map of Romania
  ...
  if(argc == 2)
  {
	  ...
	  for(size_t i =0; i < CityNames.size(); i++)
		  ...
		  if(not found)
		  {
			  cout<< "This is no city named "<<argv[1]<<"in the map!\n";
			  return(1);
		  }
  }
  ...
  // An instance of A* search class
			...
			while(SearchCount < NumSearches)
			{
				...
				do
				{
					...
					cout<< "Closed : \n";
					p = astarsearch.GetOpenListStart();
					while( p )
					{
						len++;
		#if !DEBUG_LISTS_LENGTHS_ONLY
					...
					cout<< "Closed  list has " << len << "node\n";
		#endif
					
				}
				while( SearchState == AStarSearch<PathSearchNode>::SEARCH_STATE_SEARCHING )
				
				if( SearchState == AstarSearch<PathSearchNode>::SEARCH_STATE_SEARCHED)
				{
